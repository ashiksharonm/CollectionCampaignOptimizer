...
class ThompsonSamplingPolicy(BasePolicy):
    ...
    """
    def __init__(self, n_actions: int, feature_dim: int, sigma: float = 1.0, v: float = 1.0):
        super().__init__(n_actions, feature_dim)
        self.v = v # Scaling factor for variance
        
        self.B = [np.identity(feature_dim) for _ in range(n_actions)] # Precision matrix (inverse covariance)
