...
from src.features.schemas import Channel, Customer
...
class Simulator:
    ...
    CHANNEL_COSTS = {Channel.SMS: 1.0, Channel.CALL: 5.0, Channel.FIELD_VISIT: 20.0}
    ...
    def generate_customer(self) -> Customer:
        ...
        return Customer(
            ...
            last_channel=(
                self.rng.choice([c.value for c in Channel])
                if self.rng.random() > 0.3
                else None
            ),
        )

    def _get_risk_impact(self, customer: Customer, channel: Channel) -> float:
        if customer.risk_score > 80:
            if channel == Channel.SMS:
                return -0.04
            elif channel == Channel.FIELD_VISIT:
                return 0.25
        return 0.0

    def _get_dpd_impact(self, customer: Customer, channel: Channel) -> float:
    ...
    def _get_amount_impact(self, customer: Customer, channel: Channel) -> float:
    ...
    def _get_tier_impact(self, customer: Customer, channel: Channel) -> float:
    ...
    def _calculate_recovery_prob(self, customer: Customer, channel: Channel) -> float:
        ...
        prob += self._get_risk_impact(customer, channel)
        prob += self._get_dpd_impact(customer, channel)
        prob += self._get_amount_impact(customer, channel)
        prob += self._get_tier_impact(customer, channel)

        # Channel inherent effectiveness boost
        if channel == Channel.CALL:
            prob += 0.05
        if channel == Channel.FIELD_VISIT:
            prob += 0.10
